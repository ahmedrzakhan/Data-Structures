...
// of operations. In each operation he selects a pair of adjacent lowercase 
// letters that match, and he deletes them. For instance, the string aab could 
// be shortened to b in one operation.
...
function runProgram(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    if (stack[stack.length - 1] === input[i] && stack.length !== 0) {
      stack.pop();
    } else {
      stack.push(input[i]);
    }
